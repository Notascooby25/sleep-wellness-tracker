...
missing_count="$(awk -F': ' '/Missing files:/ {print $2}' "$tmp_output" | tail -n 1)"
...
if [[ "$missing_count" =~ ^[0-9]+$ ]] && (( missing_count > 0 )); then
  ...
  echo "$ts ALERT missing_files=$missing_count report=$report_path" | tee -a "$HOOK_LOG"
  exit 2
fi

echo "$ts OK missing_files=0" | tee -a "$HOOK_LOG"
