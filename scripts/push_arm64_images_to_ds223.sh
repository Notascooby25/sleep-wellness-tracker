...
FRONTEND_LATEST="${FRONTEND_LATEST:-sleep-wellness-tracker-frontend:latest}"
...
podman tag '$FRONTEND_IMAGE' '$FRONTEND_LATEST'
podman image ls --format '{{.Repository}}:{{.Tag}}' | grep -E 'sleep-wellness-tracker-(backend|frontend):'
"
